...
export function initMetaBalls(containerSelector, options = {}) {
  const container = document.querySelector(containerSelector);
  if (!container) return;

  const {
    color = '#222222',       // Default very dark grey
    cursorBallColor = '#000000', // Default black
    speed = 0.3,
    enableMouseInteraction = true,
    hoverSmoothness = 0.05,
    animationSize = 60,      // Large animation size to make balls look scattered and smaller
    ballCount = 15,
    clumpFactor = 2.0,       // Higher clump factor spreads them out more
    cursorBallSize = 3,
  ...
  gl.canvas.style.pointerEvents = 'none'; // Ensure clicks pass through
  // Add mix-blend-mode for a cool effect if desired, but normal is fine.
  gl.canvas.style.mixBlendMode = 'multiply'; // Makes dark balls blend with background
  ...
  // We attach mouse listeners to document so it tracks over the whole screen
